...
todos:
  - id: p0-module-prefix-contract
    content: 【P0 验收门禁】拉一期禅道缺陷 JSON 验证 title【模块】前缀存在且稳定、与资料模块清单对齐；不稳定则补别名映射/未分类兜底；P0 未过不得验收
    status: pending
  - id: shared-bug-stats
...
> 6. **新增 P0 数据契约门禁**：上线/验收前必须先验证禅道缺陷「【模块】」前缀的存在性与稳定性（见 §P0）。

---

## P0、数据契约门禁（最高优先级 · 验收门禁）

> **背景**：§3（bugStats 单一事实源）与 §5（校验闸门）已能在发布时拦住错数据，但执行表与 C6 校验的底层假设是——**禅道缺陷 title 中的「【模块】」前缀存在且稳定，且能与辅助资料的模块清单对齐**。这是当前最大剩余风险：若前缀缺失或命名漂移，拼表会错配/漏配，触发 C6 硬阻断 → 报告反复发不出去。与其在发布时撞墙，不如前置验证这条数据契约。

**P0 目标**：在任何「验收完成」之前，先用真实数据验证数据契约，并据此定稿拼表规则。

**步骤**：
1. 拉取一期项目（星地多网融合 SaaS 1期）真实禅道缺陷 JSON。
2. 解析每条 title，统计：
   - 含可识别「【模块】」前缀的占比；
   - 前缀模块名集合，与辅助资料（测试方案 1.4.1 全景表）模块清单的匹配率；
   - 不匹配/缺前缀样本清单（别名、错别字、简称）。
3. 判定：
   - **稳定（达标，建议 ≥95% 有前缀且可对齐）** → 维持 §4.1 拼表规则。
   - **不稳定（不达标）** → 补充拼表规则：建立「模块别名映射表」、模糊匹配，并为无法归类的缺陷保留「未分类」兜底行 + 告警，**禁止静默丢弃**。

**门禁规则**：
- 其他 skill 改造（bug-stats、test-report-notion、校验闸门、写入层等）**可并行推进**；
- 但 **P0 未通过前，整体不得标记「验收完成」**——执行表与 C6 的正确性悬而未决。

**已有线索（需复核）**：样板页「2026-06-02」三、四节缺陷标题**均带规范【模块】前缀**，说明该约定在实际报告中已被遵循；但这是「报告文案」而非「禅道原始 JSON」，原始 title 可能存在差异，故 P0 仍须对原始 JSON 复核。
...
0. **【P0 数据契约门禁】** 拉一期禅道 JSON 验证「【模块】」前缀稳定性 → 稳定则维持 §4.1 拼表规则，不稳定则补别名映射 / 未分类兜底。**此项不通过不得验收**（其他 skill 改造可并行）。
...
> **前置门禁**：§P0 数据契约验证必须先通过，否则即使下列项全部满足也不算验收完成。
