...
Handoff：`output/runtime/handoff/latest.json`（已 gitignore）

## SDK 用法

```bash
# 安装（一次性）
cd services/qa-pipeline/mcp-server && npm i @cursor/sdk

# 环境
set CURSOR_API_KEY=你的密钥

# 全流程
node services/qa-pipeline/regression/qa-pipeline.mjs --mode full ^
  --project "【磐钴】位置监控平台-国际化" ^
  --screenshots "C:\path\screenshot.png" ^
  --no-closed

# Agent2 同时写入 Notion（子页面模式，推荐）
node services/qa-pipeline/regression/qa-pipeline.mjs --mode agent2 ^
  --project "【磐钴】位置监控平台-国际化" ^
  --no-closed ^
  --notion ^
...
node services/qa-pipeline/regression/qa-pipeline.mjs --mode agent2 --agent2-script --no-closed
```
...
- `skills/qa-agent-report-publish/SKILL.md`
- `skills/notion-test-report/SKILL.md`
