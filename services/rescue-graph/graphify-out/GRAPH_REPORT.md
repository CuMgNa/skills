...
- 页面读取：51；截断：0；含未知块：0。
- 纳入图谱：50 页；未覆盖：0 页。
- 节点域：25 个；EXTRACTED 边：141；AMBIGUOUS 边：4。
- formal 决议：10；pending 候选：0（未并入正式图）；unmapped：96。
- 边状态：legacy=94；stale=8；orphan=0（stale/orphan 查询应降权并标注待确认）。
