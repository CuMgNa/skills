...
复用 `skill/mcp/scripts/zentao-bug-create.mjs` 中已验证的链路，**不**用 `zentao upload` CLI：
...
抽出一个独立 `zentao-upload.mjs`（放在 `skill/mcp/scripts/`），只导出 `uploadImage(localPath)`，供采集器和提交器共用。登录态用环境变量 `ZENTAO_URL/ACCOUNT/PASSWORD`（与 MCP 配置一致）。
...
```bash
# 形态（实际用法）
browser-use skill/mcp/scripts/regression-shoot.py -- \
  --bug-id 3836 \
...
**实现文件**：`skill/mcp/scripts/regression-shoot.py`
...
```bash
# dry-run（默认，只打印不提交）
node skill/mcp/scripts/regression-submit.mjs --dir skill/mcp/output/handoff/regression/<bug_id>-<timestamp>

# 真正提交
node skill/mcp/scripts/regression-submit.mjs --dir <同上> --yes
```

**实现文件**：`skill/mcp/scripts/regression-submit.mjs`
...
| **`skill/mcp/scripts/zentao-bug-create.mjs`** | **抽 `uploadStepsImage` + `ensureWebSession` 成独立 `zentao-upload.mjs`，采集器/提交器共用** |
