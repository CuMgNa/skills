...
3. `assets/革泰/11.png`、`22.png`、`33.png`、`44.png` 当前 UI（2026-09-17 替换旧图 1~6.png，旧引用已同步更新）；
4. 本轮确认结论。
...
2. 设备详情编辑中的 `Location Interval` 会生成 `UPLOAD` 指令，但本篇只作关联引用，完整保存流程沿用设备管理大纲。（2026-09-20 需求变更：详情页定位频率恢复可编辑，当日早前「纯字段展示」意向口径撤销）
3. 当前账户只要对设备具有有效可见关系，即具备指令控制权；服务端仍必须按登录账号和设备唯一标识重新鉴权。
4. 单设备直接入口：离线 / 未连接设备可进入指令页并选择指令与参数，最终确认时由服务端拦截并 Toast 提示离线；批量选择页维持离线置灰不可选。两入口 UI 规则有意不对称，但服务端校验链路一致。告警设备只要底层在线，仍可下发。（2026-09-21 口径更新）
5. 批量选择页的离线设备置灰、不可勾选。
...
16. 指令页内展示设备在线状态标识（44.png 单设备指令页证据），口径为进页快照：停留期间不实时刷新，需更新只能退出重进；状态标识不联动禁用任何操作。七类指令对离线设备全部可选，不逐指令置灰，拦截统一在最终确认时由服务端裁决。（2026-09-21）
17. 提交时被离线拦截（未受理）的指令是否生成“失败”记录，归《革泰 APP 指令记录逻辑点梳理》确认，本篇不预设结论。（2026-09-21）
...
| --- | ---: | --- | --- | --- |
| 设备卡片更多 → `Send Command` | 固定 1 台 | 点击目标设备右上角更多菜单 | 当前账户可见的设备，不区分在线、离线、未连接或告警状态，均可从设备列表更多菜单进入设备指令详情页。| 当前设备指令页 |
...
2. 不允许因为入口不同，出现同一设备同一指令一个入口能发、另一个入口无业务依据地拒绝。注意：单设备入口放行离线设备进入、批量选择页离线置灰，属于入口级 UI 策略差异，不构成校验链路分叉——两入口最终都由服务端在确认时统一拦截离线设备（2026-09-21）。
...
| 离线 | ❌ | 批量模式置灰不可选；单设备入口可进入指令页（页内展示离线标识），最终确认时服务端拦截并 Toast 提示离线 |
| 未连接 | ❌ | 统一按离线处理 |
| 选择时在线、确认时离线 | 单设备：全部失败被拦截；批量：部分失败 / 拒绝 | 最终确认时服务端重新校验，不依赖列表缓存 |
...
| 同类任务冲突 | 不做跨入口「同刻仅一条」拦截，并发后发优先（2026-09-20 对齐裁决，见指令记录大纲 §6）；回文挂靠风险继承 S10U §3.1 |
...
flowchart TD
    ...
    C --> F["进入当前设备指令页（含设备摘要与进页快照在线状态标识）"]
...
→ 点击离线设备更多菜单
→ 点击 Send Command
→ 正常进入当前设备指令页（页内展示离线状态标识，指令与操作均不禁用）
→ 选择指令、设置参数
→ 点击确认下发
→ 服务端校验发现设备离线 → 拦截，不受理
→ Toast：设备离线，无法下发指令，不显示“指令已提交”
```

口径说明（2026-09-21）：

1. 入口不拦截、页内不置灰、确认时拦截——三段式边界，前端任何环节不因离线预先禁用；
2. 拦截后不留指令页常驻提示，仅 Toast；是否在指令记录中生成失败记录，归《革泰 APP 指令记录逻辑点梳理》确认（本篇 §1.3 结论 17）；
3. 设备恢复在线后，用户重新发起即可正常提交。
...
2. 最终确认接口必须重新校验；
3. 校验失败时不显示“指令已提交”；
4. 页面提示“设备离线，无法下发指令”（单设备离线拦截为 Toast 形式，不留常驻提示）；
5. 用户返回或设备恢复在线后可重新发起。
...
来源：`assets/革泰/22.png`（第 1 屏 Select Device 多选流程）。
...
来源：`assets/革泰/22.png`（第 2 屏指令选择弹层）与 PRD US-020。
...
来源：`assets/革泰/22.png`（第 3 屏 Confirm command 确认弹窗）。
...
| 当前 `assets/革泰/11.png`、`22.png` | 未提供批量关机专属确认页，无法证明长按交互已落实 |
...
来源：`assets/革泰/22.png`（第 4 屏 Command Submitted 提交成功页）。
...
当前 `assets/革泰/33.png` 记录页文案为“全部设备下发指令历史”，与本轮确认的批次上下文存在差异，需要 UI / 产品统一。`assets/革泰/1.png`（中文涂改稿）标题同为指令记录页，佐证该差异。

### 9.5 状态边界

`assets/革泰/33.png` 出现绿色勾选、“已送达”、返回版本号和“设备离线，发送失败”等内容，且记录页提供 All / Delivered / Failed 三态筛选 Tab。本篇只确认提交后可以进入记录查看，不定义这些状态的技术判定。
...
5. 离线拦截后的重试属于用户主动修正行为，不受防重复限制；每次重试仍是独立提交请求，由服务端重新校验（2026-09-21）。
...
| 编号 | 前置 / 操作 | 预期 |
| --- | --- | --- |
| CMD-01 | 离线设备更多菜单 → Send Command | 可正常进入指令页（页内展示离线标识、指令不禁用），走到最终确认时服务端拦截，Toast“设备离线，无法下发指令”，不显示“指令已提交” |
...
| CMD-14 | 设备选择时在线，最终确认时离线 | 服务端拒绝该设备；批量场景其他有效设备继续，单设备场景全部失败被拦截（与 §3.2 对齐） |
...
11. 提交时被离线拦截（未受理）的指令是否在指令记录中生成“失败”记录——归《革泰 APP 指令记录逻辑点梳理》确认，确认前本篇不预设该记录存在；
12. `assets/革泰/11.png` 批次详情页的「取消下发」「重新发送」按钮被涂改划叉（疑似设计否定）：记录页是否保留重发入口，影响 §10.3“可重试”的落地路径，需产品明确；
13. 单设备指令页的在线状态标识为进页快照、不实时刷新：快照与确认时实际状态的差异窗口是否需要页内提示补强（当前仅靠确认拦截兜底）。
...
**文档版本**：v1.3（2026-09-21 口径更新：① 单设备直接入口对离线设备放开——可进入指令页、可选指令与参数、最终确认时服务端拦截并 Toast，批量选择页维持离线置灰不可选（两入口有意不对称）；② 指令页展示设备在线状态标识，口径为进页快照、不实时刷新、不联动禁用操作；③ 七类指令对离线设备全放行，不逐指令置灰；④ 离线拦截是否建指令记录归指令记录大纲确认；⑤ 参考真源图片由旧 1~6.png 更正为 11/22/33/44.png，全文引用同步修复；⑥ §2.1 原则 2 补注入口不对称说明、§3.2、§4 流程图、§5.2/5.3、§12 CMD-01 与 CMD-14 同步新口径，待确认清单增补 3 项。此前 v1.2 的详情页可编辑口径与并发后发优先裁决保留）
