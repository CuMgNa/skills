...
2. ~~设备详情编辑中的 `Location Interval` 会生成 `UPLOAD` 指令~~ **2026-09-20 意向口径（待产品确认 US-018）**：详情页 Location Interval 改纯字段展示、不再是下发入口，改频统一走本篇 Send Command / 批量流程（§7.2）。
...
| ~~设备详情 → Edit → Location Interval → 整页 Save~~ | — | **意向口径（待产品确认 US-018）已删除该入口**：详情页改纯字段展示 | — | — |
...
4. ~~设备详情产生的 `UPLOAD` 可以有独立页面交互，但最终仍进入同一指令服务和记录体系~~ **意向口径（待产品确认 US-018）**：详情页不再是 UPLOAD 入口，改频统一走本篇流程。
...
6. 指令提交参数 ≠ 详情页展示值：详情字段仅成功反馈后更新（设备管理大纲 5.4/5.7），实际生效结果不在本篇展开。
...
**文档版本**：v1.1（2026-09-20 意向口径——待产品确认 US-018：删除「设备详情 Edit → Location Interval」下发入口引用，改频统一走本篇 Send Command / 批量流程（§7.2）；详情页改纯字段展示。否决则回滚 v1.0 口径）
