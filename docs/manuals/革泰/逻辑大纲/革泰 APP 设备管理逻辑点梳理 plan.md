...
| Location Interval | 定位频率 | 服务端保存的最后确认 `UPLOAD` 成功反馈参数 | 只读 | 仅二态：具体值（分钟-文案映射见 5.4）或 `-`（从未走通完整设置链路）；指令在飞不影响本字段 |
...
3. Location Interval 没有成功确认值时显示 `-`，与其他只读字段缺省规则一致；不能用 10 分钟默认值、目标值或「设置中」替代。
...
```mermaid
flowchart TD
    A["进入设备详情"] --> B["按 Device ID 请求最新详情与编辑权限"]
    B --> C{"请求成功？"}
    C -->|"否"| C1["保留可恢复页面状态并展示可重试原因"]
    C -->|"是"| D["按 UI 字段顺序展示；只读空值显示 -"]
    D --> E["点击 Edit 进入整页编辑模式（仅资料字段）"]
    E --> G{"返回 / 取消且存在未保存修改？"}
    G -->|"是"| G1["直接丢弃全部本地修改，不弹二次确认"]
    G -->|"否"| G2["退出编辑模式"]
```

1. Location Interval 为纯展示字段：只读、无点击行为、不进入编辑模式；改频一律走指令入口（Send Command / 批量下发 / 记录页重发，见下发大纲 §7.2）。
2. 整页 `Save` 只提交普通资料（名称、联系人、电话、分组），与任何指令下发解耦，成功 / 失败为单一结果。
3. 用户返回或取消编辑时，设备名称、联系人、电话、分组全部直接丢弃，不自动保存、不二次确认。
4. 所有当前账户可见设备均展示 Edit，并允许编辑本节列出的全部可编辑字段；服务端仍必须重新校验设备处于当前账户可见范围，不能只依赖前端按钮。

### 5.4 定位频率的数据模型与展示口径

> **2026-09-20 测试侧意向口径（待产品确认 US-018）**：详情页 Location Interval 为**纯字段展示**（只读、无编辑入口），改频一律走指令入口。产品否决则回滚 v1.7 双入口版本。

<aside>
❗

**详情字段只有两个合法取值：具体值或 `-`。** 用户选择了某个选项、平台受理了指令、指令在飞，都不能改变本字段的展示；只有「设备在线 → 指令下发 → 收到成功反馈」完整链路走通，后端才写入并返回具体值。
</aside>

| 概念 | 定义 | 详情字段表现 |
| --- | --- | --- |
| 确认值 | 最近一次走通完整链路（在线 → 下发 → 成功反馈）的 `UPLOAD` 参数，由后端保存并随详情接口返回 | ✅ 具体值（分钟-文案映射见下表） |
| 从未成功 | 平台没有任何走通完整链路的历史 `UPLOAD` 参数 | `-`（与其他只读字段缺省一致；不能推定终端没有出厂配置） |
| 指令在飞 / 失败 / 取消 | 记录处于 SENDING / SENT / FAILURE / CANCEL，或回写回跳中 | 字段**不动**：保持旧确认值或 `-`；在飞状态看指令记录页 |
...
2. 关联成功且协议将该 ACK 定义为设置成功时，服务端才把 600 秒保存为确认值；**仅此一种路径更新确认值**。
3. **频率设置即普通指令**：Send Command、批量下发、指令记录页重发三个指令入口的 `UPLOAD` 平等，只要设备在线即可下发，不做跨入口「同刻仅一条」拦截；多条并发时回文按「最新已发送指令」近似挂靠（串扰风险继承 S10U §3.1——**串扰后果在纯展示模型下是字段被写成错误值，升级为必测探针**）。
4. 仅选中参数、接口受理、成功写入指令记录、已发送但未回复，均不得覆盖确认值。
5. 定位频率与 APP 在线刷新间隔分别管理；前者控制设备位置上传周期，后者只控制 APP 页面取数。

**分钟值与展示文案映射**

| 类型 | 分钟值 | `UPLOAD` 参数 | 详情展示文案 |
| --- | ---: | ---: | --- |
| 预设一 | 1 | 60 秒 | 1 分钟（耗电模式） |
| 预设二 | 10 | 600 秒 | 10 分钟（默认模式） |
| 预设三 | 60 | 3600 秒 | 1 小时（省电模式） |
| 自定义 | 1～1440 的其他整数 | 分钟 × 60 秒 | N 分钟 |

- 「耗电模式 / 默认模式 / 省电模式」只由分钟值自动映射，不是终端另行上报的独立模式标识，也不新增运行模式命令、低电阈值或续航承诺。
- 参数选择与校验（3 预设 + 1～1440 自定义）属指令入口 UI（下发大纲 §7.2），本篇不重复。

### 5.5 定位频率与绑定、在线状态的关系

1. **定位频率移出绑定流程**：绑定只建立账户归属和分组关系，不要求填写、暂存或下发定位频率。
2. **绑定不要求设备在线**：离线设备可以完成绑定，不创建等待上线后自动执行的频率任务。
3. **刚绑定的正常初始态**：若平台没有该设备历史成功 `UPLOAD` 记录，详情页显示 `-`；不得自动显示 10 分钟。
4. **字段展示与设备在线状态无关**：离线时字段照常显示最后确认值或 `-`（纯展示不涉及下发）。

### 5.6 整页保存与防重复

整页 Save 只提交普通资料（名称、联系人、电话、分组），与指令下发完全解耦，成功 / 失败为**单一结果**，不存在部分成功。
...
sequenceDiagram
    ...
    User->>App: 编辑资料（仅普通资料字段）
    User->>App: 点击整页 Save
    App->>App: 禁用 Save，校验字段、权限
    App->>Profile: 按 Device ID 提交共享资料 + 当前账户 groupId
    alt 保存失败
        Profile-->>App: 返回失败原因
        App-->>User: 保留编辑内容并展示原因
    else 保存成功
        Profile-->>App: 返回最新资料
        App-->>User: 回显并退出编辑
    end
...
2. 服务端按本次保存操作的幂等标识去重，确保普通资料只更新一次。
3. 前端禁用不能替代服务端幂等；网络超时重试、重连或重复请求不得重复更新资料。

### 5.7 频率字段与指令四态联动（对齐 S10U / 指令记录大纲）

> 指令交互层（记录状态、网关队列、30 天丢弃、取消 / 重发）一律以 `S10U 指令交互状态逻辑点梳理.md` 与 `革泰 APP 指令记录逻辑点梳理 plan.md` 为准线。详情字段只消费四态结果，不承载在飞过程展示。

| 指令记录事件 | 详情字段表现 |
| --- | --- |
| 前置闸门拦截（不创建记录） | 不动 |
| SENDING（滞留队列）/ SENT 无回文 | 不动（旧确认值或 `-`） |
| CANCEL / CANCEL 回写 SENT（乐观取消回跳） | 不动 |
| FAILURE（复检失败 / 网关失败 / 白名单 / 30 天网关丢弃） | 不动 |
| **SENT + 匹配成功回文** | **更新为该记录参数对应文案**（唯一更新路径，不分来源入口：Send Command / 批量 / 重发） |
| 指令记录页「重新发送」创建新记录 | 在飞期间字段不动；其成功回文到达后按新参数更新（重发即新设置意图） |

1. 字段更新时机 = 后端收到匹配成功反馈并落库后，详情接口返回新值；APP 不做本地预写或乐观更新。
2. 用户已离开详情时不发送系统推送或站内消息；下次进入详情读取最新值。
...
stateDiagram-v2
    [*] --> Empty: 从未走通完整链路
    [*] --> Confirmed: 有历史成功反馈值
    Empty --> Confirmed: 任一指令入口的 UPLOAD 匹配成功反馈
    Confirmed --> Confirmed: 后续 UPLOAD 匹配成功反馈，覆盖为新参数
    note right of Empty
        展示为 -
        指令在飞/失败/取消均不改变状态
    end note
...
3. 所有可见设备均允许编辑上述共享字段与当前账户分组；定位频率为只读展示，无编辑入口。
...
2. **详情频率字段只有具体值或 `-`**：指令参数选择属于指令入口 UI，详情只展示后端确认值；没有「设置中」「目标值」中间态。
3. **无成功反馈不得更新字段**：平台受理、发送成功、SENT 无回文、用户看到提交成功提示，都不能替代终端有效反馈。
4. **`-` 不等于 10 分钟**：刚绑定、存量无记录、从未设置成功等场景均为 `-`。
5. **资料保存与指令下发解耦**：整页 Save 只管资料，单一结果；改频走指令入口，两者互不产生部分成功。
6. **频率并发后发优先**：频率设置即普通 `UPLOAD` 指令，指令入口平等、在线即可发，不设跨入口单任务锁；并发时回文按「最新已发送」近似挂靠，**串扰可能导致字段写入错误值——必测探针**（S10U §3.1）。
7. **共享资料与账户分组不能混存**：名称 / 联系人 / 电话跨账户同步，分组只影响当前账户。
8. **可见设备可编辑不等于可以越权构造请求**：服务端必须按当前登录账户与 Device ID 校验可见范围和操作权限。
...
| 上传间隔自定义 / 换算 | 属指令入口参数校验（下发大纲 §7.2），本篇仅引用 | 详见下发大纲 CMD-11 / CMD-12 |
...
| 定位频率展示 | 具体值文案（成功反馈值）、`-`（从未成功） | 无成功反馈时显示「10 分钟（默认模式）」；指令在飞时字段变化；`-` 与 10 分钟混用 |
...
8. **定位频率**：设备上传间隔与 APP 刷新间隔分别管理；详情页为只读二态展示（具体值 / `-`），仅成功反馈后更新；参数选择与下发均在指令入口（见 5.4 / 5.7）。
...
- **绑定失败提示的最终文案**：APP 已按「设备卡号无效 → 未入库 → 类型不支持 → 当前账号已绑定 → 配额不足 → 同类槽位占用」确定拦截顺序；各失败原因的最终中英文文案仍需产品 / UI 定稿。未激活、无 SIM、SIM 异常、离线或未上报不属于绑定失败条件。
- **扫码失败的最终提示文案**：本期不定义二维码内部载荷及 SN / IMEI / 设备编号的映射实现，扫码与手输最终统一取得已入库体系可识别的设备卡号；无法取得有效卡号、非本平台二维码及非牛马定位器类型的最终中英文提示仍需产品 / UI 定稿。
- **设备配额**：个人账户是否有绑定数量上限，上限值与超限提示文案。
- **解绑的欠费 / 风险拦截**：原平台监控侧对企业一级账号解绑有欠费强拦截、管理后台为风险二次确认；革泰一期是否沿用，APP 解绑是否需要同类拦截。
- **详情页定位频率改纯字段展示（2026-09-20 测试侧意向口径）**：Location Interval 改为只读二态展示（具体值 / `-`）、无编辑入口，改频统一走指令入口——动机是消除部分成功模型、离线置灰分支与双参数面板维护；**待产品确认 US-018 后生效，否决则回滚 v1.7 双入口版本**。
...
- **UPLOAD 反馈契约落库**：当前按「同设备 `UPLOAD,interval` 记录收到匹配无参数反馈，即以该指令参数更新详情字段确认值；并发时回文按最新已发送记录近似挂靠」梳理；仍需研发在接口 / 协议契约中明确反馈关联键、重复反馈与串扰挂靠的处理。串扰导致字段写错值在纯展示模型下为必测探针。
...
**文档版本**：v1.8（2026-09-20 意向口径——待产品确认 US-018：详情页 Location Interval 改**纯字段展示**（只读二态：具体值 / `-`），删除频率编辑入口、频率弹层、暂存目标值与部分成功模型；频率确认值仅成功反馈后更新，指令在飞不影响字段；改频统一走指令入口。否决则回滚 v1.7 双入口版本）
