...
| 一级分组 | 真实业务分组 | Level 1 | ✅ | ✅ | APP 可新增、重命名、删除 |
| 二级分组 | 由监控平台 / 管理后台按原逻辑创建 | Level 2 | ❌ | ✅ | APP 仅展示、搜索、筛选和选择，不可改删 |
| 三级分组 | 由监控平台 / 管理后台按原逻辑创建 | Level 3 | ❌ | ✅ | APP 仅展示、搜索、筛选和选择，不可改删 |
...
- APP 的「分组管理」页**只展示一级分组**，不展示虚拟「全部分组」，也不展示二、三级分组。
- APP 新增分组时固定创建一级分组，不提供 Level 或父组选择。
- 所有当前账户下的一级分组均可在 APP 重命名、发起删除，不区分其最初由 APP、监控平台还是管理后台创建。
- 二、三级分组仅出现在分组选择器中，供设备筛选和设备所属分组选择；不提供重命名、删除或调整父级入口。
- 分组管理页既然只展示一级组，卡片无需重复显示 Level 1 标签；展示分组名称、**直属设备数**、重命名和删除入口即可。
...
```mermaid
flowchart TD
    A["新增一级组 / 重命名一级组"] --> B{"名称长度是否为 1～40 字符？"}
    B -->|"否"| B1["禁止保存并提示长度要求"]
    B -->|"是"| C{"是否至少包含一个非空白字符？"}
...
| 重命名影响 | 即使一级组存在设备或子组仍允许改名；只更新名称，不改变分组 ID、层级、设备归属和子组关系；二级分组的父组展示同步更新 |
| 容量上限 | APP 不写死分组数量上限，沿用服务端当前配置与提示 |

### 4.5 删除规则与交互

分组管理卡片中的红色「删除」按钮始终展示。点击后先实时校验可删除条件；**只有无子组且无直属设备的一级空组才能进入二次确认并删除**。

```mermaid
flowchart TD
    A["点击一级分组的删除按钮"] --> B{"是否存在二级 / 三级子组？"}
...
旧口径「删除非空分组后设备进入未分组」已废止。本期不存在「未分组」，非空分组一律禁止删除，不能隐式迁移或删除设备。
...
| 业务组数量 | 各一级、二级、三级分组括号内均为**直接归属该组**的设备数，不包含后代分组设备 |
| 平铺方式 | 按父子关系连续平铺；一级后连续展示其二级和三级后代，再展示下一个一级 |
| 父组信息 | 二、三级分组展示其直接父组；一级不展示父组 |
...
5. **APP 只管理一级组**：二、三级由平台 / 后台维护，APP 只读展示和选择。
6. **删除不迁移设备**：有子组或直属设备均禁止删除；先校验子组，再校验设备。
7. **名称全局精确判重**：跨一级、二级、三级校验；大小写和空格差异视为不同名称。
8. **同名虚拟项与业务组可共存**：真实一级组允许命名为「全部分组」，靠固定位置和 Level 标签区分。
9. **跨端以服务端数据为准**：打开选择器 / 管理页时获取最新数据，分组失效则回退虚拟「全部分组」。
10. **本节只定义分组管理及其直接联动**；整群轨迹、围栏统计和批量指令的独立业务流程不在本节展开。
