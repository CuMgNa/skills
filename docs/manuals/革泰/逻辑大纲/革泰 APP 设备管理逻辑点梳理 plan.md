...
> **2026-09-20 需求变更回退**：详情页定位频率**恢复可编辑**（产品确认），撤销同日「纯字段展示」意向口径。保留对齐裁决：无 30 秒终判、指令入口平等后发优先、取消即解锁、重发即新设置意图。
...
| 设置中值 | 对应 `UPLOAD` 指令记录处于 SENDING / SENT 且尚无匹配成功回文的目标值 | ⚠️ 只能以「目标文案 +（设置中）」展示 |
...
3. **频率设置即普通指令**：详情页频率设置、Send Command、批量下发、指令记录页重发四个入口的 `UPLOAD` 平等，只要设备在线即可下发，不做跨入口「同刻仅一条」拦截；多条并发时后发记录成为当前展示目标值，回文按「最新已发送指令」近似挂靠（串扰风险继承 S10U §3.1，探针用例覆盖）。
4. 仅选中目标值、接口受理、成功写入指令记录、已发送但未回复，均不得覆盖最后确认值。
5. 定位频率与 APP 在线刷新间隔分别管理；前者控制设备位置上传周期，后者只控制 APP 页面取数。
6. 频率任务层**完全绑定指令记录四态**（SENDING / SENT / FAILURE / CANCEL），不设独立的 30 秒超时终判，联动规则见 5.7。
...
> 指令交互层（记录状态、网关队列、30 天丢弃、取消 / 重发）一律以 `S10U 指令交互状态逻辑点梳理.md` 与 `革泰 APP 指令记录逻辑点梳理 plan.md` 为准线。频率任务层不设独立超时终判，**完全绑定指令记录四态**：
...
4. 设置中仍允许提交新频率目标（后发优先，无单任务锁）；离线仍置灰不可提交。

| 指令记录事件 | 详情页频率展示 |
| --- | --- |
| 前置闸门拦截（不创建记录） | 保持原确认值 / Unknown，可再提交 |
| FAILURE（复检失败 / 网关失败 / 白名单 / 30 天网关丢弃） | 恢复原确认值 / Unknown，可再提交 |
| CANCEL（未写出、真正取消成功） | 恢复原确认值 / Unknown，可再提交 |
| CANCEL 回写 SENT（乐观取消回跳，设备实际会收到） | 维持设置中（目标值不变），等回文 |
| SENDING（滞留队列）/ SENT 无回文 | 目标文案 +（设置中） |
| SENT + 匹配成功回文 | 确认值更新为该记录参数，移除设置中 |
| 指令记录页「重新发送」创建新记录 | 新记录进入 SENDING，频率任务重新挂起设置中（目标 = 原记录参数）——重发即新设置意图 |

5. 确认值更新**不分来源入口**：详情编辑、Send Command、批量下发、记录页重发产生的 `UPLOAD`，任一条匹配成功回文都按其参数更新确认值。
6. 用户已离开详情时不发送系统推送或站内消息；下次进入详情读取最终结果，失败时展示一次可读提示。
...
stateDiagram-v2
    [*] --> Unknown: 无历史确认值
    [*] --> Confirmed: 有最后确认值
    Unknown --> Setting: 在线选择目标值并整页保存（记录进入 SENDING）
    Confirmed --> Setting: 在线选择目标值并整页保存（记录进入 SENDING）
    Setting --> Confirmed: 匹配成功回文（SENT + feedback）
    Setting --> Unknown: 记录 FAILURE / CANCEL 且首次设置无旧值
    Setting --> Confirmed: 记录 FAILURE / CANCEL，恢复旧值
    Setting --> Setting: CANCEL 回写 SENT，或提交新目标（后发优先）
```
...
3. 所有可见设备均允许编辑上述共享字段与当前账户分组；定位频率仅要求设备在线（并发规则见 5.4 第 3 条）。
...
7. **频率并发后发优先**：频率设置即普通 `UPLOAD` 指令，四入口平等、在线即可发，不设跨入口单任务锁；并发时回文按「最新已发送」近似挂靠，串扰风险继承 S10U §3.1。
8. **频率任务绑指令四态**：设置中 = 记录 SENDING / SENT 无回文；FAILURE / CANCEL 恢复原值；匹配成功回文才更新确认值——不存在独立的 30 秒超时终判，也不存在「设备上线不得执行」的撤回承诺（滞留队列的 UPLOAD 按网关 30 天规则自然处理）。
...
8. **定位频率**：设备上传间隔与 APP 刷新间隔分别管理；详情展示最后确认值，编辑选项只形成目标值，只有匹配成功回文后才更新确认值；频率任务层完全绑定指令记录四态（见 5.7）。
...
- **UPLOAD ACK 契约落库**：当前按「同设备 `UPLOAD,interval` 记录收到匹配无参数 ACK，即以该指令参数更新最后确认值；并发时回文按最新已发送记录近似挂靠」梳理；仍需研发在接口 / 协议契约中明确 ACK 关联键、重复 ACK 与串扰挂靠的处理。
...
**文档版本**：v1.9（2026-09-20 需求变更回退：详情页定位频率**恢复可编辑**，撤销 v1.8「纯字段展示」意向口径，内容回到 v1.7 口径——频率弹层、暂存目标值、部分成功、离线置灰恢复；同时保留 v1.7 的四态对齐裁决：无 30 秒终判、四入口平等后发优先、取消即恢复原值、重发即新设置意图）
