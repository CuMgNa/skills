...
**适用范围**：覆盖 US-005「登录后修改密码」（本篇全量收录，含异常场景）、US-026 入口项「通知提醒」（原告警设置，本篇完整梳理两开关交互；防骚扰 / 渠道等告警业务规则留告警专项）、US-028「查看和维护个人资料」、US-029「统一设置 APP 展示时区」（已独立成时区专项篇，本篇仅指针，见第 8 章）、US-030「设置监控页面刷新间隔」、US-034「查询与筛选指令记录」；US-031 退出登录收录于登录篇，本篇仅指针引用。
...
**专项已独立成篇**：`docs/manuals/革泰/逻辑大纲/革泰 APP 时区设置逻辑点梳理 plan.md`（2026-09-18 拍板全量版），本节仅留指针与速查。

速查（详见时区篇）：

- 入口：我的 → 时区设置（区域三）；与登录页选定时区是**同一份纯本地账号级配置**（IANA ID），资料页时区行只读回显。
- 交互：点选即生效（对勾单选、无保存按钮）；下次渲染全 APP 生效。
- 列表：系统 tzdata + Windows 式呈现（38 档 / 138 项），偏移标注随访问日期动态（含 DST）。
- 默认：首次登录 / 注册预选手机系统时区。
- 🔴 已知风险 RISK-TZ-01（已实测验证）：下载轨迹接口固定偏移传参，跨 DST 相位历史轨迹导出时间错 1 小时——契约缺陷非 bug，处置待产品拍板（时区篇第 10 章）；轨迹查询/下载链路另有关联风险群 RISK-TZ-02~05 待取证（时区篇第 10.3 章）。
...
- `docs/manuals/革泰/逻辑大纲/革泰 APP 时区设置逻辑点梳理 plan.md`：US-029 时区设置专项（2026-09-18 独立成篇）；含已知风险 RISK-TZ-01。
...
**文档版本**：v1.3（第 8 章时区设置压缩为指针：US-029 独立成时区专项篇；其余不变）
